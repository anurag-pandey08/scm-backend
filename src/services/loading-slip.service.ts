...
export const loadingSlipService = {
  ...
  /**
   * Strikes out every slip the clerk ticked, and says how many went.
   *
   * The count can be short of the list. A book is a page the office is
   * reading, and a slip deleted at the next desk between the tick and the
   ...
   * and it reads as a 404 the same way asking for one missing slip does.
   */
  async removeMany(slug: string, ids: string[]): Promise<number> {
    const company = await companyService.getRow(slug);
    const deleted = await loadingSlipRepository.deleteMany(company.id, ids);

    if (deleted === 0) {
      throw AppError.notFound("None of those slips are in this book");
    }
